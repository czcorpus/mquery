...
func (gen *VerbObjectQGen) FxQuery(word Word) string {
	if word.PoS == "" {
		return fmt.Sprintf(
			...
			gen.SketchConf.FuncAttr.Name, gen.SketchConf.NounObjectValue,
...
func (gen *VerbObjectQGen) FxQuerySelectSQL(word Word) (sql string, args []any) {
	if word.PoS == "" {
		sql = fmt.Sprintf("SELECT f.result, f.result_type FROM %s_scoll_query AS q "+
			...
			gen.CorpusName, gen.CorpusName, gen.SketchConf.LemmaAttr.Name, gen.SketchConf.PosAttr.Name,
...
func (gen *VerbObjectQGen) FxQueryInsertSQL(word Word, result *rdb.WorkerResult) (sql string, args []any) {
	...
	sql = fmt.Sprintf(
		...
		gen.CorpusName, gen.SketchConf.LemmaAttr.Name, gen.SketchConf.PosAttr.Name,
		gen.SketchConf.FuncAttr.Name, gen.SketchConf.ParPosAttr.Name,
	)
	var val string
...
func (gen *VerbObjectQGen) FyQuerySelectSQL(collCandidates []string) (sql string, args []any) {
	placeholders := ""
	...
	args = append(args, gen.SketchConf.NounObjectValue, gen.SketchConf.VerbValue)
